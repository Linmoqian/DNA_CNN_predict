...
class GeneExpressTransformer(nn.Module):
    ...
    def __init__(self, num_classes: int = 2):
        ...
        # --- Halflife 分支 ---
        self.hl_fc1 = nn.Linear(8, 48)
        self.hl_fc2 = nn.Linear(48, 48)
    ...
    def forward(self, promoter: torch.Tensor, halflife: torch.Tensor) -> torch.Tensor:
        ...
        # 融合压缩
        x = self.pool2(F.relu(self.bn_fuse(self.conv_fuse(x))))  # (B, 48, ~312)

        # Token 压缩: (B, 48, ~312) -> (B, 48, 64) -> (B, 64, 48)
        x = self.token_pool(x)
        x = x.permute(0, 2, 1)  # (B, 64, 48) 即 64 个 token, d_model=48

        # 加位置编码
        x = x + self.pos_embed

        # Transformer 全局自注意力
        x = self.transformer(x)  # (B, 64, 48)

        # 全局平均池化
        x = x.mean(dim=1)  # (B, 48)

        # Halflife 分支
        h = F.relu(self.hl_fc2(F.relu(self.hl_fc1(halflife))))  # (B, 48)

        # 融合分类
        x = torch.cat([x, h], dim=1)  # (B, 96)
