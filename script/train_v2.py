...
from model.modelv2 import GeneExpressNet
...
def main():
    ...
    train_loader = DataLoader(
        TensorDataset(train_p, train_h, train_l), batch_size=32, shuffle=True
    )
    valid_loader = DataLoader(
        TensorDataset(valid_p, valid_h, valid_l), batch_size=32
    )
    test_loader = DataLoader(
        TensorDataset(test_p, test_h, test_l), batch_size=32
    )

    # 模型
    model = GeneExpressNet(num_classes=2).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    print(f"模型: {C.hi('GeneExpressNet')}  参数量: {C.bold(f'{total_params:,}')}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), data_dir / "modelv2_best.pt")
            saved = f"  {C.ok('saved')}"
            no_improve = 0
        else:
            no_improve += 1
    ...
    model.load_state_dict(torch.load(data_dir / "modelv2_best.pt", weights_only=True))
